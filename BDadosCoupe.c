...
/** \brief Adicionar dados(registos) a uma tabela, os dados são dados numa string onde o separador é ‘;’
 *
 * \param BD BDadosCoupe*
 * \param nome_tabela char*
 * \param dados char*
 * \return int
 *
 */
...
/** \brief F)	Pesquisar uma Tabela da base de dados
 *
 * \param BD BDadosCoupe*
 * \param nome_tabela char*
 * \return TABELA*
...
/** \brief Memória ocupada por toda a base de dados.
 *
 * \param BD BDadosCoupe*
 * \return long int
 *
...
int Exportar_BDados_Ficheiro_Binario(BDadosCoupe *BD, char *fich_dat)
{
    ...
    //nome BD
    char *s_nome_BD = (char*) malloc(sizeof(char) * strlen(BD->NOME_BDADOS));
    ...
    free(s_nome_BD);
    //versao BD
    char *s_versao_BD = (char*) malloc(sizeof(char) * strlen(BD->VERSAO_BDADOS));
    ...
    while(P)
    {
        TABELA *T=P->Info;
        //nome tabela
        char *s_nome_tabela = (char*) malloc(sizeof(char) * strlen(T->NOME_TABELA));
        strcpy(s_nome_tabela,  T->NOME_TABELA);
...
int Importar_BDados_Ficheiro_Binario(BDadosCoupe *BD, char *fich_dat)
{
    ...
    fread(&numero_tabelas, sizeof(int), 1, F);
    TABELA *T;
    ...
    for(int i=0; i < numero_tabelas; i++)
    {
        ...
        char *s_tabela = (char*) malloc(sizeof(char) * N_tabela + 1);
...
int DELETE_TABLE_DATA(TABELA *T)
{
    ...
    T->LRegistos = CriarLG(); // Cria a lista de novo, mas fica vazia
    DestruirLG(T->LCampos, Destruir_Campo);
    T->LCampos = CriarLG(); // Cria a lista de novo, mas fica vazia
    exe = clock() - exe;
    double tempo = ((double)exe)/CLOCKS_PER_SEC;
    FILE *f1 = fopen("estatisticas.csv", "a");
    fprintf(f1,"%s;%.3f\n",__func__,tempo);
...
/** \brief Função nao parte da biblioteca,  MOSTRA TODOS OS REGISTOS DE UMA TABELA
 *
 * \param T TABELA*
 * \return int
 *
...
BDadosCoupe *Ler_nome_versao_BD_bin(char *fich_dat)
{
    ...
    char *s_nome_BD = (char*) malloc(sizeof(char) * N_nome_BD + 1);
    ...
    char *s_versao_BD = (char*) malloc(sizeof(char) * N_versao_BD + 1);
 ...
 *
 */
int Condicao_Maior(char *info1, char*info2)
{
    if(!info1 || !info2) return INSUCESSO;
    return stricmp(info1, info2) > 0 ? SUCESSO : INSUCESSO; // caso o stricmp devolva um valor menor 0, X1 é maior alfabeticamente
}
 ...
 *
 */
int Condicao_Menor(char *info1, char *info2)
{
    if(!info1 || !info2) return INSUCESSO;
    return stricmp(info1, info2) < 0 ? SUCESSO : INSUCESSO; // caso o stricmp devolva um valor menor 0, X1 é menor alfabeticamente
}
 ...
 *
 */
int Condicao_Igual(char *info1, char *info2)
{
    if(!info1 || !info2) return INSUCESSO;
...
/** \brief A condição para retornar SUCESSO caso a string de referencia seja ao converter para int, maior que a  que a pesquisada numericamente
 * ISSO SERÁ NECESSÁRIO PARA O SELECT, DELETE, E UPDATE.
 *
 * \param info1 void*
 * \param info2 void*
 * \return int
 *
 */
int Condicao_Maior_Numericamente(char *info1, char *info2)
{
    if(!info1 || !info2) return INSUCESSO;
...
/** \brief A condição para retornar SUCESSO caso a string de referencia seja ao converter para int, menor que a  que a pesquisada numericamente
 * ISSO SERÁ NECESSÁRIO PARA O SELECT, DELETE, E UPDATE.
 *
 * \param info1 void*
 * \param info2 void*
...
/** \brief A condição retorna sucesso caso encontre a substring dentro da string de comparação, por exemplo
 * para a string "Informatica" se passarmos a substring "Info" será verdadeiro, pois "Info" está dentro de "Informatica"
 * \param str char*
 * \param substr char*
 * \return int SUCESSO / INSUCESSO
 *
 */
int Condicao_Contem_Palavra(char *str, char *substr)
{
